...
func Run(ctx context.Context, opts Options) error {
	...
	// Drop legacy user-level hooks only AFTER the authoritative drop-in is in
	// place, and never fail setup over it: the drop-in already provides
	// coverage, so a corrupt or unwritable ~/.claude/settings.json is at worst
	// duplicate hook events the user can clear by re-running setup.
	if err := removeLegacyUserHooks(); err != nil {
		fmt.Fprintf(opts.Stderr, "warning: could not remove legacy Claude Code hooks (%v); managed hooks are active — re-run setup or clear ~/.claude/settings.json to avoid duplicate hook events\n", err)
	}
