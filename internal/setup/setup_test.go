...
func TestRunRefusesExistingManagedHooksWithStaleSelfServeConfig(t *testing.T) {
	h := newHarness(t)
	allowLoopback(t)
	if err := os.MkdirAll(filepath.Dir(managedconfig.UserPath()), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(managedconfig.UserPath(), []byte(`{
...
func TestRunFailsBeforeManagedDropInWhenLegacyHookCleanupFails(t *testing.T) {
	h := newHarness(t)
	allowLoopback(t)
	...
	err := Run(context.Background(), h.options("tok", server))
	if err == nil || !strings.Contains(err.Error(), "clean legacy Claude Code hooks") {
		t.Fatalf("Run() error = %v, want legacy cleanup failure", err)
	}
	if _, err := os.Stat(managedSettingsPath); !os.IsNotExist(err) {
		t.Fatalf("managed settings written despite cleanup failure: %v", err)
	}
}
